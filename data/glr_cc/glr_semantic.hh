...
class yy_semantic_simple_value : public yy_semantic_value_abstract
{
  YYSTYPE& get_value()
  {
    return yyvalue;
...
class yy_semantic_option : public yy_semantic_value
{
private:
...
public:
  yy_semantic_option(yy_glr_state* rhs, yy_rule_num r, int c,
                     bool has_lookahead, yy_glr_semantic_option* next
  ...
  virtual void destroy(const std::string& msg, int type]b4_locuser_formals([yylocp])[))
  {
    ...
    yy_glr_state *yyrh = yystate;
    for (int yyn = yyrhsLength (yyrule);
         yyn > 0;
         yyrh = yyrh->yypred, --yyn)
      yyrh->yydestroyGLRState (yymsg]b4_user_args[);
  }
  ...
  bool operator== (const yy_semantic_option& yyy1) const;

  /** Assuming identicalOptions (YYY0,YYY1), destructively merge the
   *  alternative semantic values for the RHS-symbols of YYY1 and YYY0.  */
  void yymerge_option_sets (yy_semantic_option& yyy1);

  /** Y0 and Y1 represent two possible actions to take in a given
   *  parsing state.  */
  yypreference_value yypreference (yy_semantic_option& y1);
  ...
  YYRESULTTAG
  yyreportAmbiguity (yy_semantic_option& yyx1]b4_pure_formals[) const;
